...
NOSE = 0
  ...
  RIGHT_FOOT_INDEX = 32
"""
diz = {"curl":0, "squat":0, "Push-Up":0, "plank":0}
action = None
reps = 0
msg = ""
time_diff = None
...
actions = np.array(['curl', 'squat','Push-Up','null'])
...
def gen_frames():
    ...
    global msg
    global time_diff
    last_down = {'curl':None, 'squat':None, 'Push-Up':None, 'plank':None }
                            ...
                            if results.pose_landmarks:
                                ...
                                if action == 'Push-Up':
                                    if (l_curlangle < 90 or r_curlangle < 90) :
                                        stage = "down"
                                        if not last_down['Push-Up']:
                                            last_down['Push-Up'] = time.time()
                                    if l_curlangle > 150 or r_curlangle > 150 and stage == "down":
                                        stage = "up"
                                        if last_down['Push-Up'] is not None:
                                            time_diff = round(time.time() - last_down['Push-Up'],4)
                                            msg = classify_speed(time_diff, 'Push-Up')
                                            last_down['Push-Up'] = None
                                        counter += 1
                                        diz[action] += 1
                                    
                                    listOfStrings = actions_state[-10:]
                                    if len(listOfStrings) > 0:
                                        if all(elem == 'Push-Up' for elem in listOfStrings) and counter == 0:
                                            if reps == 0:
                                                clock = time.time()
                                            action = "plank"
                                            reps += 0.2
